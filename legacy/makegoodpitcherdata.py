...
def loadindata():
    ...
    start = time.clock()
    with open('MLBGoodDataOD2010thruASB2016.pkl','rb') as input:
...
def makepitcherdata(Data):
    ...
    allovr = np.asarray(range(0,len(Data.Inning)))

    inds2014plus = np.where(Data.gameID > 201400000000)[0]
    
    print len(inds2015plus)
    
    #print Data.gameID
    
    indsj2016 = np.where(Data.gameID > 201606000000)[0]
    
    #pitcherVec = np.asarray(list(set(Data.pitcher[indsj2016])))
    
    pitcherVec = np.asarray([453562, 453286, 518516, 433587, 477132, 434378, 446372, 572971, 431148])
    
    #pNums = np.zeros([len(set(Data.pitcher[indsj2016])),1])
    pNums = np.zeros([len(set(pitcherVec)),1])
    
    for i in range(0,len(pitcherVec)):
        pNums[i] = len(np.where(Data.pitcher[inds2015plus] == pitcherVec[i])[0])
    ...
    for k in range(0,len(pitcherVec)):
        ...
        pInds = inds2015plus[np.where(Data.pitcher[inds2015plus] == p)[0]]
        ...
        batmat = np.zeros([len(outpitchgood),18])
        #start = time.clock()
        ...
        YearCounts[8:15,k] = getcount(outpitchgood)[:,0]
        ...
        str1 = '%i ' * 19 + '%2.2f ' + '%i ' + '%2.1f ' * 3 + '%5.6f ' * 71 + '%5.6f'

        str2 = '%i ' * 9 + '%5.6f ' * 6 + '%i %i %i %i'

        np.savetxt('PitcherData/' + str(p) + 'Data2014.txt',pDataMatrix,str1)
        np.savetxt('PitcherData/' + str(p) + 'Outputs2014.txt',pOutMatrix,'%i %2.2f %i %i')
        #np.savetxt('PitcherData/YearCountStuff.txt',YearCounts.T,str2)
